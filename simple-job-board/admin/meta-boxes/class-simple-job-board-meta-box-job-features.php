...
class Simple_Job_Board_Meta_Box_Job_Features {
     ...
     * 
     * @since   2.2.3
     * @since	2.9.6	Added esc_attr() to echoing variables.
                ...
                if (NULL != $keys):
                    foreach ($keys as $key):
                        if (substr($key, 0, 11) == 'jobfeature_') {
                            ...
                            echo '<input type="text" id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '[value]" value="' . esc_attr( $value ) . '" />';
                            ...
                            if ($job_post_layout_version == 'v2') {
                                ...
                                echo '&nbsp;&nbsp;<input type="text" id="icon_' . esc_attr( $key ) . '" class="sjb-job-feature-icon" name="' . esc_attr( $key ) . '[icon]" value="' . esc_attr( $icon_value ) . '" /><span class="input-group-addon"><i class="' . esc_attr( $icon_value )  . '"></i></span> ';
                            }
                    ...
                    foreach ($removed_options as $key => $val):
                        if (substr($key, 0, 11) == 'jobfeature_') {
                            ...
                            if ($job_post_layout_version == 'v2') {
                                if (isset($val['icon']) && '' !== $val['icon']) {
                                    $icon_value = $val['icon'];
                                } else if (isset($removed_options['icon_' . $key])) {
                                    $icon_value = $removed_options['icon_' . $key]['icon'];
                                ...
                                echo '<input type="text" id="" class="" name="' . esc_attr( $key ) . '[icon]" value="' . esc_attr($icon_value) . '"/><span class="input-group-addon"><i class="' . esc_attr($icon_value) . '"></i></span> ';
                            }
                            echo '<div class="button removeField">' . esc_html__('Delete', 'simple-job-board') . '</div></li>';
                        }
                    endforeach;
